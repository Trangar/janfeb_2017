...
impl TextGraphics {
    pub fn new(display: &GlutinFacade) -> Result<TextGraphics> {
        ...
        let font = Rc::new(FontTexture::new(display, &include_bytes!("../../assets/arial.ttf")[..], 16)?);
        Ok(TextGraphics {
            system: system,
            font: font,
            cache: HashMap::new(),
    ...
    pub fn draw_at(&mut self, frame: &mut Frame, name: String, screen_width: f32, screen_height: f32, x: f32, y: f32, color: Color) -> Result<()> {
