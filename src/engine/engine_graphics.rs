use glium::{Blend, DisplayBuild, DrawParameters, IndexBuffer, Frame, VertexBuffer, Program,
            Surface};
...
impl<T: TGraphicIndex> EngineGraphics<T> {
    ...
    pub fn draw(&mut self, key: T, x: f32, y: f32, rotation: f32, scale: f32) -> Result<()> {
        if let Some(ref helper) = self.graphics.get(&key) {
            if let Some(ref mut frame) = self.frame {
                EngineGraphics::<T>::draw_at(&self.textured_program,
                                             frame,
                                             helper,
                                             self.width,
                                             self.height,
    ...
    fn draw_at(textured_program: &Program,
               frame: &mut Frame,
               helper: &DrawHelper,
               width: f32,
               height: f32,
        ...
        frame.draw(&helper.vertex_buffer,
                  &helper.index_buffer,
                  textured_program,
                  &uniform,
                  &draw_parameters)?;
